...
#define EXAMPLE_DEVICE_STRING L"\\Device\\WfpDrvFirewall"
#define EXAMPLE_DOS_DEVICE_STRING L"\\DosDevices\\WfpDrvFirewall"
...
NTSTATUS DriverEntry(IN PDRIVER_OBJECT driver_obj, IN PUNICODE_STRING registry_path)
{
	...
	driver_obj->MajorFunction[IRP_MJ_DEVICE_CONTROL] = DriverDispatch;
...
NTSTATUS DriverDispatch(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
{
	...
	// pointer to the current location in the Irp
	...
	{
	case IRP_MJ_CREATE:
		DbgPrint("DrvFirewall.SYS: IRP_MJ_CREATE\n");
		break;

	case IRP_MJ_CLOSE:
		DbgPrint("DrvFirewall.SYS: IRP_MJ_CLOSE\n");
		break;

	case IRP_MJ_DEVICE_CONTROL:
		DbgPrint("DrvFirewall.SYS: IRP_MJ_DEVICE_CONTROL\n");
		...
		default:
			...
			DbgPrint("DrvFirewall.SYS: unknown IRP_MJ_DEVICE_CONTROL\n");
			break;
		}
	...
	WDFDRIVER* driver, WDFDEVICE* device)
{
	...
	WdfDeviceInitSetDeviceType(device_init, FILE_DEVICE_DRVFIREWALL);
...
VOID DriverUnload(PDRIVER_OBJECT driver_obj)
{
	...
	DbgPrint("--- WFPDriver driver unloaded ---");
	return;
}
